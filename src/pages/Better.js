...
function Better({ activeChain, connectedAddress }) {
    ...
    return [...Array(BINS).keys()].map(
      k => <>
          ...
          <input 
            ...
            id={"inputField" + k}
            key={"inputField" + k}
            onChange={betsInputHandler(k)}></input>
  ...
  function betsInputHandler(k) {
    return (e) => {
      setBetSizes(betSizes?.map( (v, i) => 
      i === k
      ? v.add(ethers.utils.parseUnits(e.target.value, 18))
      : v));
    }
  }
  ...
  const placeBets = useCallback(
    () => 
    {
      ...
      console.log("bets input:", betSizes.map(x => x.toString()))
      executePlaceBets?.({
  ...
  const epochDelta = useCallback(() =>
    (Math.floor(Date.now() / 1000)) - (lastEpochClose + epochDuration + bufferDuration),
  ...
  return (<>
    ...
    {(new Date((lastEpochClose + epochDuration + bufferDuration) * 1000)).toLocaleString()}
