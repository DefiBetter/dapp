...
export default function PublicSale() {
  ...
  const { write: approvePaymentTokenWrite } = useContractWrite({
    ...
    args: [presaleContractConfig.address, ethers.constants.MaxUint256.sub("1")],
    onError(error) {
      console.error(error);
  ...
  useContractRead({
    ...
    args: [connectedAddress, presaleContractConfig.address],
    onSuccess(data) {
      setAllowance(data);
    },
    watch: true,
  ...
  useContractRead({
    ...
    args: [presaleContractConfig.address],
    onSuccess(data) {
  ...
  return (
    <div className="relative bg-db-light dark:bg-db-dark-nav transition-colors rounded-md p-2 md:p-4 min-h-[86vh]">
      <PageTitle title={"Public"} fancyTitle={"Sale"} />
